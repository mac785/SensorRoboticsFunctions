function [thetalist, success, iter] = J_inverse_kinematics(robot, T_desired, thetalist0, ...
                                                            eomg, ev, max_iter)
...
%   iter      - number of iterations taken
    ...
    for iter = 1:max_iter
        % Current end-effector transform (body form)
        T_curr = FK_body(robot.M, robot.Blist, thetalist);

        % Body twist needed to reach T_desired from T_curr
        % [V_b] = log( T_curr^{-1} * T_desired )  in se(3)
        V_b_mat = MatrixLog6(inv_transform(T_curr) * T_desired);
        V_b     = [SO3ToVec(V_b_mat(1:3,1:3)); V_b_mat(1:3,4)];  % 6-vector
        ...
        % Newton-Raphson update: theta += J_b^+ * V_b
